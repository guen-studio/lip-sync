...
class LIPSYNC_SpriteSheetAnimator:
    ...
    #-------------------------------------------------------------------
    # Unoffical set_interpolation function by Guen Studio
    # ------- pt-br
    # Blender 5.x introduziu o sistema de Actions Layered.
    # Nas versões anteriores (Blender 4.x e anteriores), as curvas de animação
    # podiam ser acessadas diretamente através de:
    #
    #     action.fcurves
    #
    # No Blender 5.x as FCurves passaram a ficar armazenadas dentro de um
    # ActionChannelbag associado a um ActionSlot, localizado em:
    #
    #     Action -> Layer -> Strip -> ChannelBag -> FCurves
    #
    # Por esse motivo o código foi adaptado para localizar o slot de animação
    # utilizado pelo addon e obter as FCurves através do ChannelBag.
    #
    # Objetivo desta função:
    # Alterar a interpolação de todos os keyframes gerados pelo Lip Sync para
    # CONSTANT, evitando transições interpoladas entre visemas e preservando
    # a troca instantânea de sprites/poses da boca.
    # Blender 5.x introduced the Layered Actions system.
    # ------- en-us
    # Blender 5.x introduced the Layered Actions system.
    #
    # In previous versions (Blender 4.x and earlier), animation curves could be
    # accessed directly through:
    #
    #     action.fcurves
    #
    # In Blender 5.x, FCurves are now stored inside an ActionChannelbag associated
    # with an ActionSlot, following the hierarchy:
    #
    #     Action -> Layer -> Strip -> ChannelBag -> FCurves
    #
    # For this reason, the code was updated to locate the animation slot used by
    # the addon and retrieve the FCurves through the corresponding ChannelBag.
    #
    # Purpose of this function:
    # Set the interpolation mode of all keyframes generated by the Lip Sync addon
    # to CONSTANT, preventing interpolation between visemes and preserving
    # instantaneous sprite/pose transitions.
    def set_interpolation(self, obj: BpyObject):
        ...
        :return: None
        """
        #-------- comment suggested by guen studio
        #Sets the interpolation mode of all keyframes in the object's animation action
        #to 'CONSTANT'.
        #
        #Supports both the legacy Action API (Blender 4.x and earlier) and the Layered
        #Actions API introduced in Blender 5.x.
        #
        #param obj: The object whose animation keyframe interpolation will be modified.
        #type obj: BpyObject
        #return: None
                 ...
                 for layer in action.layers:
                    ...
                    for strip in layer.strips:
                        ...
                        try:
                            ...
                            print("Erro ao obter channelbag:", e)
                            continue
                        ...
                        if hasattr(channelbag, "fcurves"):
                            for fcurve in channelbag.fcurves:
                                for keyframe in fcurve.keyframe_points:
                                    keyframe.interpolation = 'CONSTANT'
    #End unoffical set_interpolation
    #-------------------------------------------------------------------
